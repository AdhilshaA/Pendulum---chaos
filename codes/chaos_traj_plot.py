...
for i in tqdm(range(start_trial, end_trial + 1)):
    ...
    # first conver it to (0, 360) range for 90_180 mode
    if mode == '90_180':
    ...
    plt.plot(t1, theta1, '--o', markersize = 3, label = r'trial{}-$\theta_1$'.format(i))
    plt.plot(t1, theta2, '--o', markersize = 3, label = r'trial{}-$\theta_2$'.format(i))
    plt.xlabel('time (s)')
