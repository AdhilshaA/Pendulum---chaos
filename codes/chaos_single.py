...
file_name1 = 'chaos31' # name of the datafile1(no .txt)
file_name2 = 'chaos41' # name of the datafile2(no .txt)
mode = '90_180' # '90_180' or '90_0' or '170_0'
...
le_guess = 4 # guess for le

save_fig = True # save the figures or not (True/False)

check_best_fit = True # check best fit or not (True/False)
manual_pts = 20 # no. of points to fit for best fit if check_best_fit is False

late = []
# set 1
used = [31,38,39,40,41] # use these trials only for (90,180) position
late = [31,32,37,41]
...
if check_best_fit:
    ...
    for i in range(4, n_points + 1):
        t_now = t[:i]
        trajectory_diff_now = trajectory_diff[:i]
        
        popt, perr, chi_squared, red_chi_squared = fit_exponential(t_now, trajectory_diff_now, trajectory_diff_now[0], le_guess)
        
        print(f'({i} points): red_chi = {red_chi_squared}, le = {popt[0]} +/- {perr[0]}, d0 = {popt[1]} +/- {perr[1]}')
...
plt.xlabel('t (s)')
plt.ylabel('Phase space difference')
plt.title(r'LE_fit - chaos{}_{} - $d_0$ = {:.3f}'.format(file_name1[5:], file_name2[5:], trajectory_diff_now[0]))
plt.yscale('log')
plt.legend()
